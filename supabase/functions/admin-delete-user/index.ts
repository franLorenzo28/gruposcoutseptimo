...
serve(async (req) => {
  ...
  try {
    ...
    if (targetRole === "admin" && role !== "admin") {
      return new Response(JSON.stringify({ error: "Los moderadores no pueden eliminar administradores" }), {
        status: 403,
        headers: corsHeaders,
      });
    }
    ...
    if (userId === user.id) {
      return new Response(JSON.stringify({ error: "No puedes eliminarte a ti mismo desde el panel admin" }), {
        status: 400,
        headers: corsHeaders,
      });
    }
